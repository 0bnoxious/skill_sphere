use bevy::{
    ...
    prelude::*,
    render::{
        mesh::{self, Indices, PrimitiveTopology, VertexAttributeValues},
...
fn setup_skillsphere(
    ...
    let tetra_vertices = vec![
        ...
        Vec3::new(-(1. / (2. * f32::sqrt(3.))), 0.0, tetra_lenght / 2.),
        Vec3::new(-(1. / (2. * f32::sqrt(3.))), 0.0, -(tetra_lenght / 2.)),
    ];

    // Normals
    let tetra_indices = Indices::U32(vec![
        0, 2, 1, 0, 3, 2, 0, 1, 3, 1, 2, 3, //bottom
    ]);
...
fn setup_camera(mut commands: Commands) {
    commands
        .spawn((
            Camera3dBundle {
                transform: Transform::from_xyz(-2.0, 1.0, 5.0).looking_at(Vec3::ZERO, Vec3::Y),
                ..default()
            },
